...
function openInDesktopWindow(e) {
  ...
  // desktop host (plain web / official browser mode) the anchor opens
  // chat.deepseek.com in a NEW tab (its href carries target="_blank"), so the
  // current harness page is never replaced.
...
function createEntry() {
  ...
  a.href = CHAT_URL;
  // Plain browser / official browser mode: the anchor default must open a NEW
  // tab (target="_blank") instead of navigating the current harness page away.
  // In the desktop path openInDesktopWindow() preventDefault()s, so this is a
  // pure downgrade default and never competes with the native window flow.
  a.target = "_blank";
...
// 两代客户端设置契约并存：
//   0.1.6：inject 'settingsScope' → ctx.settingsScope.bind({ namespace })
//   0.1.7：inject 'configForms'   → ctx.configForms.get(entryId)
// 两者控制器 API 同构（getSnapshot/subscribe/set），ChatEntryPolicy 原样可用。
//
// 顶层 inject 只能写两版都有的 'slots'：把任一设置服务名写进顶层 inject，
// 会让另一版本永远 pending（0.1.7 下表现为 "waiting for service: settingsScope"，
// 整个客户端半不激活——不只是设置项，侧边栏入口也会消失）。
// 用 ctx.inject([...]) 起两个子插件做「二选一」等待，恰好只有一个会跑。
var inject = ["slots"];
function apply(ctx) {
  var registerSlots = function (policy) {
  ...
  ctx.inject(["configForms"], function (child) {
    registerSlots(new ChatEntryPolicy(child.configForms.get(SETTINGS_NAMESPACE)));
  });

  // ── 0.1.6 分支 ──
  ctx.inject(["settingsScope"], function (child) {
    registerSlots(new ChatEntryPolicy(child.settingsScope.bind({ namespace: SETTINGS_NAMESPACE })));
  });
}
